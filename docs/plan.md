...
| 源 | 读数 | 判读 |
|---|---|---|
| Workers Logs `track` 打点（按 CST 分天） | ~~08-19 loginbase 24 / logto 141（14.5%）；08-20 至 17:20 loginbase 29 / logto 69（29.6%）~~ ⚠ **数字已于 08-21 证伪作废**（宽窗口 ABR 采样，见第三次读数）；订正值：08-19 **32.4%**、08-20 **18.6%** | **只有 08-19 起是真实流量**——08-15~18 的 12%→62% 高占比全是发版前回归测试，与 `sessions` 建链数逐一吻合。⚠ 这是**调用量**占比不是用户占比，新轨用户全是刚登录的活跃用户，会放大分子 |
| 消费方客户端埋点（日活设备口径） | 新版设备占比 08-18 2.5% → 08-19 3.4% | 渗透率的正确读数。与上一行差一个量级不矛盾（调用量 vs 设备数） |
| 消费方 D1 | `sessions` 08-18 起 5/4/4 用户建链；剔除本人测试号后 **6** 个真实用户（原记 7 个，其中一个于 08-21 确认是本人另一账号），其中 2 个是存量用户命中原 `user_id` | 存量映射生产实证有效 |
...
5. **宽窗口会静默启用 ABR 自适应采样，返回的是抽样估算值而非真实计数**（2026-08-21 查明的根因，此前两次误判都源于它）。**判据在 `result.run.statistics.abr_level`：`1` = 精确，`>1` = 抽样**——7 天窗口实测 `abr_level: 10`，所有计数都是 10 的整数倍，小量事件被整体采没（真值 24 次的一类事件返回 **0**）。这也解释了此前观察到的两个怪象：宽窗口下 `view:"events"` 严重截断（calculations 计 20 条、events 只吐 1 条），以及 `groupBy` 计数自相矛盾（同一批日志按 `userId` 分组总数 8、按另两个字段各 20）。**正确姿势：把 timeframe 切成 ≤6h 的片逐片查、每片断言 `abr_level == 1` 再累加**；窄窗口下两个视图结果一致，可互为验证。宽窗口 `groupBy` 只配看「有没有」和「大概几个量级」，**不要拿它做逐日趋势表、穷举或指认具体记录**——2026-08-20 据此误判过「诊断日志没记到」，2026-08-21 又发现第二次读数的逐日占比表整个作废。⚠ 推论：**受采样污染的历史数字无法事后修复**（日志只留 7 天），要保住趋势基线只能每天窄窗口取一次数并落盘。
...
### 第三次读数（2026-08-21，距上次 28 小时）

结论：**新轨仍零错误、存量映射累计 3 例全部命中原 `user_id`、绑定冲突无第 2 例；渗透率 3.4% → 6.0%（首个完整自然日翻倍）。**
**本次同时查明上次源 1 数字整体作废的根因（ABR 采样，见备忘 5），并按新方法重取。**

| 源 | 读数 | 判读 |
|---|---|---|
| Workers Logs `track` 打点（≤6h 窄窗口分片 + `abr_level==1` 断言 + events 逐条二次验证） | 08-19 **32.4%**（60/125）· 08-20 **18.6%**（24/105）· 08-21 **15.4%**（24/132，至 21:50） | 与上次表格不可调和（上次"半天 29"大于本次"全天 24"），根因是采样。样本每天仅几十次，**逐日抖动由一两个重度用户决定，不宜从中读趋势** |
| 消费方客户端埋点（日活设备口径） | 新版设备占比 08-18 2.0% → 08-19 3.5% → 08-20 **6.0%** → 08-21 6.1%（半天） | 08-20 是首个完整自然日，渗透率翻倍。**这仍是渗透率的唯一正确读数** |
| 消费方客户端登录漏斗（1.3.0，08-18 起累计） | 面板曝光 26 · 发起 10 · 成功 8 · 取消 4 · **失败 0 / 错误 0**；设备维度 15 台触达面板 → 8 台成功（53%） | 老版本同期仍有 `sign_in_error` 7 + `sign_in_failed` 3。**新轨错误恒为 0，是两轨最硬的质量差** |
| 消费方 D1 | `sessions` 89 行 / 15 用户；1.3.0 发版以来真实用户累计 **8 个**，其中 **3 个**是存量用户走 GitHub 登录命中原 `user_id` | 存量映射零例外 |
| email 哨兵 SQL | **91**（基线 94，↓3） | ✅ 只降不升，「无洞」假设仍成立 |
| 绑定冲突 | 28 小时窗口内 `link_attempt` / `link_conflict_detail` 均为空，`oauth_callback` 仅 2 条且都是 `issued` | 上次那例 `github_in_use` **无第 2 例**，样本仍 N=1 |

**一个要记的反直觉观察（消费方风险面在扩大，不是收缩）**：未迁移的 GitHub 老用户从 **203 涨到 207**
（其中「老号无 email、选邮箱登录必然建新号」这层稳定在 90 人，「老号有 email、取决于输入哪个邮箱」
从 113 涨到 117）。**老版本 App 每天仍在持续产生新的 GitHub 用户**，他们升级后同样面临这个岔路。
所以「等发生率上来再决定要不要处置」这个策略要附带自觉：**分母每天都在变大，而渗透率只有 6%，
真正的撞击高峰还没到。** 处置仍属消费方业务规则，未拍板。

> 完整读数（含真实用户邮箱与 `user_id`、逐日明细、消费方侧待办）在**私有**仓库：
> `TrendingProjects/loginbase-双轨读数-2026-08-20.md` （该文件含第二次与第三次两次读数）。本文档为 public，故此处只留脱敏结论。
