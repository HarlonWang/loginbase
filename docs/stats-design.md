...
| 档 2 上报端点形态 | ⏭ 推二期 |
| 客户端事件与服务端事件同表？ | ✅ 同表，`source` 列区分；该列一期就建好。**2026-09-11（2.0.0）落地**：不在 `auth_events` 内同表，而是整体写进 eventbase 的 `events` 表。`auth_events` 的写入路径**整条删除**，不留双写与回退——回滚靠降级 npm 版本，不靠库里留一份旧代码 |
