...
| 档 | 做法 | 解锁 |
|---|---|---|
| 档 1 | 在**已有请求**上带客户端标识（版本、平台） | App 版本 / 平台两个切片轴。**已实现（1.9.0，2026-09）**：头 / start 参数 → `client_version` / `client_platform` 两列，契约见 protocol.md「客户端标识」节 |
...
`source` 列现在就要有（哪怕一期不写客户端事件）：客户端数据本质不完整，**必须能与服务端权威数据区分**，否则会污染分母。`client`（版本/平台）列等真做客户端上报时再 `ALTER TABLE ADD COLUMN`，SQLite 上这是便宜操作——**1.9.0 已加**：`client_version` / `client_platform`（migration 0003），值只来自客户端结构化上报，未上报为 NULL、读数时单独成桶。
...
**复用既有的 `db`**——不引入新 binding、不加任何依赖，与「依赖准入」铁律无冲突。
...
> 取数 SQL 见 [../queries/core-metrics.sql](../queries/core-metrics.sql)，执行方式与五条易读错处见 [../queries/README.md](../queries/README.md)。自动化（日报/告警）暂不做——流量还没起来，先人肉。
...
#### 6.13 首次真实读数（2026-08-17 18:00~19:31，TrendingAI 测试流量）

上线当天就有一波真人测试跑过完整链路，**10 条核心指标里 8 条真的出了数**（另 2 条 C3/G1 为空是因为没有失败发生，不是缺数据）。

| 项 | 读数 | 判读 |
|---|---|---|
| 规模 | 3 个独立用户 / 2.6 小时 / 149 事件 / 23 次登录（github 18、email 5） | `session_revoked:current` 22 次与登录数近 1:1 = 反复登出再登的测试模式，**不能当真实用户基线** |
| 交叉验证 | `login` 23 = OAuth 兑换 18 + 验码 5 | 两条轨的口径对得上，统计链路端到端可信 |
| D4 ①→③ 流失 | 5 / 39（12.8%） | 其中一对最可疑：17:10:57 与 17:12:10 连续两次都是 `CN/AMS`——中国电信绕到阿姆斯特丹，73 秒后重试仍失败，`github.com` 授权页在那条链路上大概率打不开 |
| D11 回跳丢失 | 1 / 19（5.3%） | 19:28:47 `CN/LAX`；**前一分钟 19:27:00 还有一次 start 落在 `JP/KIX`**——一分钟内 country 与 colo 双双变化，说明用户正在切换网络/代理，回跳丢失极可能是这个动作的副产品（~~推断，非定论~~ 2026-08-31 证伪：全历史 42 个丢失 flow 的 country/colo 全程未变，反而完成 flow 里有 19 次变了——切网不是诱因） |
| **D13 回跳耗时** | 中位约 1.7s / 最快 446ms / **最慢 10.0s**，`over_45s = 0` | **otc 的 60 秒 TTL 确认够用**，还有 6 倍余量。这条指标当初没算进 v1，是 `flow_id` + `at` 白送的 |
| E2 绑定冲突 | 15 次到达 callback：14 冲突（全部 `github_in_use`）+ 1 成功 | **link 流程首次得到验证**（CLAUDE.md 里长期挂着的「未验」项），冲突判定、reason 透传、回跳参数整条链正常 |

**最有价值的发现：国内流量全部绕到海外边缘节点。**

| 网络 | colo | 次数 |
|---|---|---|
| CN / AS56041（中国移动） | **LHR 伦敦** | 64 |
| CN / AS56041 | **SEA 西雅图** | 35 |
| CN / AS56041 | LAX / SJC | 16 / 12 |
| CN / AS4134（中国电信） | **AMS 阿姆斯特丹** | 2 |
| HK / AS45102（阿里云） | HKG | 18 |

`country=CN` 的请求**没有一次落在国内节点**，且同一运营商在 2.6 小时内落到四个不同大洲——这既解释了国内用户的延迟来源，也说明路由不稳定。**这个维度靠的是 `colo`，就是 6.11 里被判为「零隐私、白捡」的那个字段。**

**一条教训**：当时的哨兵（只盯故障）在这段时间里**返回空**，而实际发生了 14 次 `link_conflict`。它是「预期业务结果」不假，但密集出现就是消费方业务规则的体检信号。哨兵因此改为 A/B 两档（见 `queries/core-metrics.sql`）。**覆盖面只能被真实数据检验出来，纸上想不全。**
...
| 档 1 客户端标识 header | ✅ 1.9.0 落地（migration 0003 两列；服务端不从 UA 解析，CLAUDE.md 铁律） |
| 档 2 上报端点形态 | ⏭ 推二期 |
| 客户端事件与服务端事件同表？ | ✅ 同表，`source` 列区分；该列一期就建好。**2026-09-11（1.10.0）落地**：不在 `auth_events` 内同表，而是整体写进 eventbase 的 `events` 表——配 `stats.db` 即切换，不配则仍写 `auth_events`（退役路径，保留以便回滚）。两者只写一处不双写 |
| 客户端漏斗标识如何对齐 | ✅ **2026-08-31 改判（1.7.0）：方向反转——客户端标识上行**（start 的可选 `client_flow_id` 参数，随 state 透传全链进 `meta.clientFlowId`），而非原设想的「复用服务端 `flow_id`」。原因：服务端 `flow_id` 客户端拿不到（exchange 响应体刻意剥离统计字段），且下发也覆盖不了「从没完成」的 flow——恰是回跳丢失排查最需要的那批。同批新增 `browser_tier` / `browser_pkg`（含义见 protocol.md） |

**L4 非功能**

- **`ip` / `ua` 存不存**——v1 不存。核心指标一条都不要求（只有 H 组滥用指标需要，全是备选），纯粹是隐私与排障价值的权衡，将来是 `ALTER TABLE ADD COLUMN`
  - **2026-08-31 部分改判（1.6.1）**：OAuth 浏览器腿（`oauth_start` / `oauth_callback`）的 `ua` 与 `secChUa`（Brave 等套壳 UA 与 Chrome 一字不差，靠品牌列表区分）进 `meta`，不加列——排障价值已具体化：D11 回跳丢失定位靠浏览器身份。`sessions.user_agent` 对 OAuth 登录记的已是 callback 的浏览器 UA，但只覆盖走到 issued 的 flow；`meta.ua` 的增量在 start 腿（「从没到 callback」的 flow 此前完全没有浏览器信息）与免 join 的逐事件读取。`exchange` 不记；**`ip` 维持不入库**
