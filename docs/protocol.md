...
给应用商店审核用（Play 与 App Store 共用同一个，故邮箱别取带商店名的名字）。**客户端零改动**——它看到的就是一次普通的邮箱验证码登录，演示码与真码同形（6 位数字）。命中配置的那个邮箱时，上面两个端点的行为变化如下：

| 端点 | 演示邮箱的行为 | 与常规路径的差别 |
|---|---|---|
| `/code/send` | 恒 `200 { "cooldownSeconds": 60 }`，**与常规成功响应逐字节相同** | 不发信、不写 KV、**不计任何一层限流** |
| `/code/verify` + 正确演示码 | 与常规成功路径**完全一致**（就是走的常规路径） | 中间件替审核员写入一条临时码后放行，常规路径照常读取并焚毁；**固定码本身不受影响，可无限次重复使用** |
| `/code/verify` + 错误码 | `400 { "error": "code_expired" }` | 回落常规路径；因 send 从未存过码，结果与「未发过码的普通邮箱」逐字节一致，**不泄露该邮箱是演示账号** |

三处短路缺一不可——商店要求凭据「随时可用、可重复使用」，而常规路径的限流（单 IP 10 次/3600s）、验证成功即焚、5 次错误即焚，每一条都会破坏这一点。

演示登录**不另起一套建会话逻辑**：命中正确演示码时中间件把码写进 KV 再放行，由常规 handler 自己验过去，因此会话语义、事件、响应形状与真实登录逐字节同源。本次请求的所有统计事件（含常规 handler 在放行后发出的 `code_verify(ok)` 与 `login`）都带 `meta.demo = true`。

⚠️ **已知敞口：演示码可被暴破**（2026-08-22 知情接受，暂不处理）。`/code/verify` 本身无限流——常规路径防暴破靠「错 5 次焚码」+「码 600s 过期」，而这两条正是演示账号为满足「可重复使用」必须拿掉的。6 位固定码 + 无限次尝试 + 永不过期 ≈ 100 万种组合可穷举；攻破代价限于这一个演示账号，不涉及真实用户数据。收口做法是**按 IP** 记失败次数（切勿按邮箱全局，否则攻击者打满即可把审核员锁在门外）。响应体已做到与常规逐字节相同，但**时序侧信道修不掉**（演示邮箱不发信故更快），判断为可接受。

邮箱比对 `trim().toLowerCase()`，大小写不敏感。**未配置 `demoAccount` 时该路径完全不存在**，同一组邮箱和码走常规路径、换不到会话。相关统计事件带 `meta.demo = true`，便于把审核流量从登录漏斗里剔除。
...
- **1.5.0**（2026-08）：新增**可选**服务端配置 `demoAccount`，给应用商店审核用的固定邮箱 + 固定验证码（见上方「演示账号」）。**wire 零变更、客户端零改动**——演示码与真码同形，客户端看到的就是一次普通的邮箱验证码登录；**未配置时该路径完全不存在**。动因：无密码登录交不出商店要的「随时可用、可重复使用」凭据。Play 的「登录详细信息」与 App Store Connect 的「App 审核信息」要求一致，故**一个演示账号跨商店共用**；Play 的表单尤其不接受「部分功能受限但不提供凭据」，TrendingAI 1.4.1 于 2026-08-22 因此被拒。
