...
**`auth_events` 表也归库所有**（1.4.0 起，登录统计；DDL 见 `migrations/0002_auth_events.sql`，指标与口径见 [stats-design.md](stats-design.md)）：单表存 7 类事件（`code_sent` / `code_send_failed` / `code_verify` / `login` / `oauth_start` / `oauth_callback` / `oauth_exchange` / `refresh`），列为 `at / event / outcome / provider / user_id / flow_id / is_new_user / country / source / meta`。三点值得单记：
...
**旧键 `locale` 直接删、不留兼容，且不为此推 major**（2026-08-14 定，发 1.3.0）：受影响面实测为零——两个消费方都没用过 `templates`；`email.locale` 只有 TrendingAI 配了 `'zh'`，而它走 loginbase 的客户端尚未发布，**这个配置项至今没渲染过任何一封生产邮件**（商店版走 Logto 托管页发信）。兼容层是为零个受益者付复杂度。**代价在升级时自己扛**：消费方多为裸 JS Worker、没有类型检查兜底，旧键会被静默忽略。**2026-08-14 实测把这个风险说清楚了**（github-ai-trending-api 升级时反向验证）：

- 留着旧键 `locale: "zh"` 不改 → **无害**，键被忽略、兜底落到库内置 `en`；只有当消费方**本来就想要非 en 的兜底**时才算退化；
- 真正的错法是**把旧值照搬到新键**（`fallbackLocale: "zh"`）——那会把「一刀切某语言」原样带过来，正是这次要消除的东西。

所以升级动作的重点不是"别忘了改名"，而是**重新决定兜底语言应该是什么**。冒烟断言要能区分：断言「上报中文时收到的 subject 含中文、不上报时含英文」，而不是「接口 200」——后者在语言退化时同样成立，等于没验证。
...
**`onLinked`**（1.2.0，见 plan.md 第 4 步议题 2）——**已登录用户绑定第二身份**的语义出口。与 `onVerified` 的方向相反：`onVerified` **返回** userId（用外部身份换会话），`onLinked` **接收** userId（用已有会话认领外部身份）。
