...
- **邮件语言与模板体系**（原「邮件 locale」挂起项）：**`loginbase@1.3.0` 已发布**（2026-08-14，真包冒烟 14 条断言通过）——`locale`→`fallbackLocale` 正名（旧键直接删、不留兼容）、`templates` 改按语言分表 + 部件级、请求级 `locale`。客户端 `localeProvider` 已实现并合并（loginbase-kt PR #4），**未发 Maven**。**含配置 API 的 BREAKING 但不推 major**：受影响面实测为零（走 loginbase 的客户端尚未发布，`locale: 'zh'` 至今没渲染过生产邮件）。方案见 server-design.md「语言与模板体系」+ design.md 客户端节。消费方 github-ai-trending-api 已升级（[PR #36](https://github.com/HarlonWang/github-ai-trending-api/pull/36) 待审：依赖钉死 1.3.0、删掉 `locale: 'zh'` 且**不配 `fallbackLocale`**、加 5 个锁邮件语言的测试）。**剩余**：合并部署 → 再发 TrendingAI PR #99（顺序 A：客户端首版即会上报语言，故不存在「不传 locale 的世代」）。
