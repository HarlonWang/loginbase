...
多 App 共用的登录底座（邮箱验证码 + 社交 OAuth + 会话管理）。**开工前先读 README.md 和 `docs/design.md`**——路线选择、API 草案、分发方式、落地五步全在里面；命名相关看 `docs/naming.md`（含死名单，别重新讨论命名）；项目由来看 `docs/logto-替换方案-调研.md`。
...
- npm 分发走 git-tag 依赖（`github:HarlonWang/loginbase#semver:...`），**仓库必须保持 public**（Cloudflare Workers Builds 装不了私有 git 依赖）。
- KMP 分发走 R2 静态 Maven（计划 `maven.harlon.wang`），iOS target 只能在 macOS 构建。

## 当前状态

骨架 + 文档阶段，`src/`、`kotlin/`、`test/` 尚未创建。下一步 = 落地第 1 步（见 design.md 落地顺序）。
