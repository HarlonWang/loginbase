...
export interface LoginConfig {
   ...
   *
   * ⚠️ **已知敞口：演示码可被暴破**（2026-08-22 知情接受，暂不处理）。
   * `/code/verify` 本身没有限流——常规路径防暴破靠的是「错 5 次焚码」+「码 600s 过期」，
   * 而这两条正是演示账号为满足「可重复使用」必须拿掉的。于是 6 位固定码 + 无限次尝试
   * + 永不过期 ≈ 100 万种组合可穷举。攻破的代价是这一个演示账号（及其 Pro 权益与
   * AI 配额），不涉及任何真实用户数据。
   *
   * 若要收口，**按 IP 记失败次数**（如每 IP 每小时 20 次），保留不焚码、不过期：
   * 审核员复制粘贴一次即成功，正常路径无感。**切勿按邮箱全局计数**——那样攻击者
   * 打满失败次数就能把审核员锁在门外，等于我们自己造一个拒绝服务。
   * 加长验证码这条路走不通：客户端把输入硬编码成 6 位数字（满 6 位自动提交），
   * 而演示账号存在的前提正是「让已发布的客户端能过审」。
   *
   * 另有一条**修不掉的时序侧信道**：演示邮箱的 send 不发信（快），普通邮箱要走
   * 邮件服务商（慢），响应时间可区分。响应体已做到逐字节相同，时间差要消除只能
   * 人为补延迟，不值得；上面的暴破一旦收口，知道「哪个是演示邮箱」也不构成攻击。
   */
  demoAccount?: {
    /** 演示邮箱，比对时大小写不敏感 */
    email: string;
